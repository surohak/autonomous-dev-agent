...
Log: $(basename "${LOG_FILE:-?}")"
    ...
    # Immediate Tempo suggestion on successful implementation runs. The watcher
    # also fires one on the Code-Review status transition (~up to 2 min later),
...
" 2>/dev/null || true
    ...
    if [[ "$verdict" == "Done" && -n "${FORCE_TICKET:-}" ]]; then
      ...
      if declare -F tempo_suggest_now >/dev/null 2>&1; then
        tempo_suggest_now "$FORCE_TICKET" \
          "Dev done on ${FORCE_TICKET}. Log dev time?" >/dev/null 2>&1 || true
      fi
    fi
  fi
}
...
3. If it's a task (e.g. 'list my MRs', 'summarize PROJ-972', 'show tests failing on 1920'),
