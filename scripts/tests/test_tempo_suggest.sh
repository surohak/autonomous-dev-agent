...
#
# PROJ-100:  1 manual impl run, 1h45m (rounded from 6300s)
#          + 1 manual impl run, 30m (rounded from 1800s) → summed → 2h15m
# PROJ-101:  1 manual impl run, 5m → below 15-min floor → dropped
# PROJ-102:  1 manual impl run, 10h → capped at 8h
# PROJ-103:  1 review round (ready → mr_approved) 45m
#          + 1 dev run 1h → total suggest 1h45m
# PROJ-104:  1 scheduled (manual=0) run 2h → ignored
# PROJ-105:  1 manual run with mode="ask" → ignored (not in DEV_MODES)
# PROJ-106:  orphan agent_start, no end → ignored, doesn't crash
# PROJ-107:  review_ready with no closer in-day → ignored
# PROJ-108:  manual impl run on a DIFFERENT day (2026-04-14) → not counted for 04-15

: > "$LOG"

# PROJ-100 — two runs summed
log '{"ts":"2026-04-15T08:00:00Z","type":"agent_start","run_id":"r100a","ticket":"PROJ-100","mode":"implementation","manual":1,"mr_iid":"--"}'
log '{"ts":"2026-04-15T09:45:00Z","type":"agent_end","run_id":"r100a","exit":"0"}'
log '{"ts":"2026-04-15T14:00:00Z","type":"agent_start","run_id":"r100b","ticket":"PROJ-100","mode":"implementation","manual":1,"mr_iid":"--"}'
log '{"ts":"2026-04-15T14:30:00Z","type":"agent_end","run_id":"r100b","exit":"0"}'

# PROJ-101 — below floor
log '{"ts":"2026-04-15T10:00:00Z","type":"agent_start","run_id":"r101","ticket":"PROJ-101","mode":"implementation","manual":1,"mr_iid":"--"}'
log '{"ts":"2026-04-15T10:05:00Z","type":"agent_end","run_id":"r101","exit":"0"}'

# PROJ-102 — hit DEV_CAP at 8h
log '{"ts":"2026-04-15T00:00:00Z","type":"agent_start","run_id":"r102","ticket":"PROJ-102","mode":"implementation","manual":1,"mr_iid":"--"}'
log '{"ts":"2026-04-15T10:00:00Z","type":"agent_end","run_id":"r102","exit":"0"}'

# PROJ-103 — review 45m + dev 1h
log '{"ts":"2026-04-15T09:00:00Z","type":"review_ready","ticket":"PROJ-103","mr_iid":"2100"}'
log '{"ts":"2026-04-15T09:45:00Z","type":"mr_approved","ticket":"PROJ-103","mr_iid":"2100"}'
log '{"ts":"2026-04-15T11:00:00Z","type":"agent_start","run_id":"r103","ticket":"PROJ-103","mode":"implementation","manual":1,"mr_iid":"2100"}'
log '{"ts":"2026-04-15T12:00:00Z","type":"agent_end","run_id":"r103","exit":"0"}'

# PROJ-104 — scheduled (manual=0) → ignored
log '{"ts":"2026-04-15T13:00:00Z","type":"agent_start","run_id":"r104","ticket":"PROJ-104","mode":"implementation","manual":0,"mr_iid":"--"}'
log '{"ts":"2026-04-15T15:00:00Z","type":"agent_end","run_id":"r104","exit":"0"}'

# PROJ-105 — ask mode → ignored
log '{"ts":"2026-04-15T16:00:00Z","type":"agent_start","run_id":"r105","ticket":"PROJ-105","mode":"ask","manual":1,"mr_iid":"--"}'
log '{"ts":"2026-04-15T16:30:00Z","type":"agent_end","run_id":"r105","exit":"0"}'

# PROJ-106 — orphan start
log '{"ts":"2026-04-15T17:00:00Z","type":"agent_start","run_id":"r106_orphan","ticket":"PROJ-106","mode":"implementation","manual":1,"mr_iid":"--"}'

# PROJ-107 — open review window
log '{"ts":"2026-04-15T18:00:00Z","type":"review_ready","ticket":"PROJ-107","mr_iid":"2200"}'

# PROJ-108 — different day
log '{"ts":"2026-04-14T08:00:00Z","type":"agent_start","run_id":"r108","ticket":"PROJ-108","mode":"implementation","manual":1,"mr_iid":"--"}'
...
for t in ("PROJ-101", "PROJ-104", "PROJ-105", "PROJ-106", "PROJ-107", "PROJ-108"):
    assert t not in tickets, f"{t} should have been dropped, got {tickets.get(t)}"

# PROJ-100: 6300 + 1800 = 8100s → rounded to quarter → 8100 (already on quarter)
#         = 2h15m.
s = tickets["PROJ-100"]
...
# PROJ-102: 10h → DEV_CAP (8h = 28800s)
s = tickets["PROJ-102"]
assert s["suggested_seconds"] == 28800, s
assert s["dev_seconds"] == 28800, s

# PROJ-103: dev 3600 + review 2700 = 6300 → 1h45m suggestion
s = tickets["PROJ-103"]
...
assert "dev run" in tickets["PROJ-100"]["description"], tickets["PROJ-100"]
assert "review" in tickets["PROJ-103"]["description"], tickets["PROJ-103"]
...
# 8: --ticket filter
# ---------------------------------------------------------------------------
OUT=$(run_suggest --ticket PROJ-100)
python3 - <<PY
import json
d = json.loads(r'''$OUT''')
assert len(d) == 1 and d[0]["ticket"] == "PROJ-100", d
print("scenario 8 OK")
PY
...
import json
d = json.loads(r'''$OUT''')
assert len(d) == 1 and d[0]["ticket"] == "PROJ-102", d
...
{"PROJ-100:$DAY":"2026-04-16T08:00:00Z"}
EOF
...
assert "PROJ-100" not in tickets, f"PROJ-100 should be skipped, got {tickets}"
# PROJ-102 and PROJ-103 should still be present.
assert "PROJ-102" in tickets and "PROJ-103" in tickets, tickets
print("scenario 9 OK")
PY
rm -f "$FIX/cache/tempo-skipped.json"

# ---------------------------------------------------------------------------
# 10: --date selects a different day → PROJ-108 appears, PROJ-100 disappears.
# ---------------------------------------------------------------------------
...
d = json.loads(r'''$OUT''')
tickets = {s["ticket"] for s in d}
assert tickets == {"PROJ-108"}, tickets
...
# ---------------------------------------------------------------------------
# 11 + 12 already covered implicitly by scenarios 1–7 (PROJ-106, PROJ-107).
# ---------------------------------------------------------------------------
...
import json
d = json.loads(r'''$OUT''')
# PROJ-101 had 5min → below floor → should now appear with skip reason set.
ua101 = [s for s in d if s["ticket"] == "PROJ-101"]
assert ua101, d
