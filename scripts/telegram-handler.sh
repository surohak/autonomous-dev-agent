...
for r in results:
    ...
    if cq and cq.get('from', {}).get('id') == chat_id:
        ...
        #   'rv_post:123:4'         -> 'rv_post 123 4' (review-related)
        #   'ci_fix:ssr:2019'       -> 'ci_fix ssr 2019' (watcher actions)
        #   'fb_fix:ssr:2019'       -> 'fb_fix ssr 2019'
        #   'fb_seen:ssr:2019'      -> 'fb_seen ssr 2019'
        #   'tk_start:UA-123'       -> 'tk_start UA-123'
        #   'tk_later:UA-123'       -> 'tk_later UA-123'
        #   'snooze:3600'           -> 'snooze 3600'
        #   'tm_log:UA-997:2026-04-15:5400'  -> 'tm_log UA-997 2026-04-15 5400'
        MULTI_SPLIT_PREFIXES = ('rv_', 'ci_', 'fb_', 'tk_', 'rel_', 'tm_')
        if data_val.startswith(MULTI_SPLIT_PREFIXES):
            text_cmd = data_val.replace(':', ' ')
        else:
            text_cmd = data_val.replace(':', ' ', 1)
        # Capture the message_id so handlers that want to edit the card in
        # place (e.g. tm_log → "Logged 1h30m — Undo") can do so. It's OK for
        # older handlers to ignore it.
    ...
    # 2a) Old 'Reply with review feedback for UA-XXX:' — MR author asking agent for fix
    m = re.match(r'^Reply with review feedback for (' + os.environ['TICKET_KEY_PATTERN'] + '):', reply_text)
    if m:
        ticket = m.group(1)
        msgs.append({'text': f'review {ticket}: {text}'})
        continue

    # 2b) Edit a pending review comment: 'Edit comment <mr_iid>#<idx>:'
    ...
    # 2c) Discuss a review comment with AI: 'Discuss comment <mr_iid>#<idx>:'
    ...
    # 2c2) Tempo duration edit: 'How long for UA-XXX on YYYY-MM-DD?'
    # Produced by handler_tm_edit via tg_force_reply. The user's reply is a
    # free-form duration string (e.g. '1h30m', '45m', '2h'); the actual
    # parse-and-log happens in handler_tm_edit_reply, which our 'tm_editapply'
    # dispatch case calls.
    m = re.match(r'^How long for (' + os.environ['TICKET_KEY_PATTERN'] + r') on (\d{4}-\d{2}-\d{2})', reply_text)
    if m:
        ticket, date = m.group(1), m.group(2)
        # Preserve the user's exact text so _tempo_parse_duration can handle
        # variants ('1h 30m', '1.5h', '90', '90m'). Tab/newline would break
        # the downstream bash read, so collapse whitespace defensively.
        payload = ' '.join(text.split())
        msgs.append({'text': f'tm_editapply {ticket} {date} {payload}'})
        continue

    # 2d) Reply to any message that starts with "Thinking…" or "Agent:" →
    # continue the chat thread with the same FORCE_PROMPT flow.
    if reply_text.startswith(('Thinking', 'Agent:', 'Chat reply')):
        msgs.append({'text': f'ask {text}'})
        continue

    # 2e) Shortcut: a message starting with '?' is a quick ask
...
print(json.dumps({'messages': msgs, 'next_offset': max_id + 1 if max_id > 0 else 0}))
" 2>/dev/null)
...
# Require current assignee = me AND status = Done
jql = f"key in ({batch}) AND assignee = '{owner_jira_aid}' AND status = 'Done'"
...
# Drop tickets that didn't satisfy (assignee=me AND status=Done)
by_key = {k: v for k, v in by_key.items() if k in matching_keys}
...
for slug, meta in repos.items():
    ...
    if not local or not os.path.isdir(local):
        continue
    mrs = []
    # Opened is glab's default state. Try JSON (-F json) first, fall back to text parsing.
...
# Persist existing-open-MR info to promoted.json so [DM Lei] button works for these too
...
PYEOF
      ...
      if [[ "$CHERRIES_RESULT" == INFO:* ]]; then
        ...
        # Send one message per ticket with a [Cherry-pick] button
        COUNT=$(printf '%s\n' "$CHERRIES_RESULT" | tail -n +2 | wc -l | tr -d ' ')
        send_telegram "$COUNT ticket(s) eligible for cherry-pick to main:"
...
if open_mr:
    ...
    buttons_row1 = [
        {"text": "DM Lei",      "callback_data": f"rel_dm:{d['key']}"},
        {"text": "Open MR",     "url": open_mr["web_url"]},
    ]
    buttons_row2 = [
        {"text": "Open in Jira","url": f"{os.environ["JIRA_SITE"]}/browse/{d['key']}"},
    ]
...
$SHIP_RESULT"
    ...
    tk_cherry\ *|cherry\ *)
      ...
      TK_KEY=$(echo "$CMD" | awk '{print $2}' | tr '[:lower:]' '[:upper:]')
      [ -z "$TK_KEY" ] && { send_telegram "Usage: /cherry UA-XXX"; continue; }
      ...
      if [[ "$CHERRY_RESULT" == OK:* ]]; then
        # Primary success message
        send_telegram "$CHERRY_RESULT"
        # Follow-up with action buttons (DM Lei / Open MR / Open in Jira)
        FOLLOWUP=$(
          TK_KEY="$TK_KEY" \
          CONFIG_PATH="$SKILL_DIR/config.json" \
          CACHE_DIR="$CACHE_DIR" \
...
PYE
      ...
      elif [[ "$CHERRY_RESULT" == INFO:*"already has an open promote MR"* ]]; then
        # MR to main already exists → show the same follow-up buttons (DM Lei / Open MR / …)
        send_telegram "$CHERRY_RESULT"
        FOLLOWUP=$(
          TK_KEY="$TK_KEY" \
          CONFIG_PATH="$SKILL_DIR/config.json" \
...
PY
    ...
    tm_editapply\ *)
      ...
      # the user types a custom duration in reply to "How long for UA-XXX
...
# Mark end of this iteration for wake-detection + reset UPDATES for next loop.
LAST_LOOP_END=$(date +%s)
UPDATES=""
