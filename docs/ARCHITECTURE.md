...
Linear), opens MRs/PRs on your code host (GitLab / GitHub), and talks
back over chat (Telegram / Slack). Runs as a `launchd` tick every
...
|---------|---------------------------------------------|----------------------------|
| tracker | `tracker_search/get/transition/comment/…`   | jira-cloud, github-issues, linear |
