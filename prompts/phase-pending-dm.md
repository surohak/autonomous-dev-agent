...
curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
  ...
  -d "$(python3 -c 'import json,sys; print(json.dumps({"chat_id":int(sys.argv[1]),"text":sys.argv[2]}))' "$TELEGRAM_CHAT_ID" "Agent: DM sent to <reviewer.name> for {{TICKET_PREFIX}}-123")" \
  > /dev/null
```
