...
reload_plist() {
  ...
  launchctl load  "$path" && echo "  loaded  : $label"
}
...
if [[ -f "$tg_tmpl" && -f "$SKILL_DIR/config.json" ]]; then
  ...
  mapfile -t tg_rows < <(python3 - <<'PY' 2>/dev/null
import json, os, sys
...
if (( SKIP_SWIFTBAR == 1 )); then
  ...
  echo "  SwiftBar not detected (no $SWIFTBAR_PLUGINS) — skipping"
